    ...
    public class DesktopAutomationService : IDisposable
    {
        ...
        public string WriteText(string fieldName, string value, string? specialKeys = null, string? windowName = null)
        {
            try
            {
                ...
                // STRATEGY 1: Background (Value Pattern)
                // Preferred because it doesn't steal focus or require the window to be on top.
                // Note: SetValue() replaces the entire text (implicit "Select All + Paste").
                // We only use this if there are no 'specialKeys' (like {ENTER}) which require keyboard simulation.
                ...
                // STRATEGY 2: Foreground (Input Simulation)
                // Fallback if the element doesn't support patterns or if we need to send special keys.
                element.SetForeground();
                element.Focus();
                Wait.UntilInputIsProcessed(); 

                // Send Special Keys (using our helper for consistency)
                ...
                // Type the value (if ValuePattern failed above)
                if (!string.IsNullOrEmpty(value))
                {
                    // We try ValuePattern again here just in case focus helped, 
                    // but usually we default to typing for compatibility.
